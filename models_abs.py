...
class PropagationNetwork(nn.Module):
    ...
    def forward(self, attr, state, Rr, Rs, Ra, node_r_idx, node_s_idx, pstep, ret_feat=False):
        ...
        if self.args.tube_mode==1 or self.args.box_only_flag:
            pred_obj[:, 0] = torch.mean(pred_obj[:, 0].view(pred_obj.size(0), -1), 1).view(pred_obj.size(0), 1, 1)
            pred_obj[:, 3] = torch.mean(pred_obj[:, 3].view(pred_obj.size(0), -1), 1).view(pred_obj.size(0), 1, 1)
